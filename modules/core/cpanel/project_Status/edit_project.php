...
$crumbs[1]['url'] = $root_path.'modules/core/admin/project_Status/index.php?menuid='.$menuid;
    ...
    if (isset($_POST['submitBtn'])) {
		...
		$sql=$christCMS->update_cont_status_single($statusID,$status,$langCode,$description);
		...
		//echo $statusID;
		//$statusID=$_GET['statusID'];
		$sqlgetn=$christCMS->get_cont_status_single($statusID,$langCode);
		$resultn = $christDB->f_ExecuteSql($sqlgetn);
		$recordcount = $christDB->f_GetSelectedRows();
		$arrProjectStatus = $christDB->f_GetRecord($resultn);
...
<div>
		  ...
		  <input type="hidden" name="id" id="id" value="<?=$statusID?>" >
            <table align="center"cellspacing="1" cellpadding="5" border="0" width="70%" >
              <tr><td><?=$LBcontTitle?>:</td><td><input name="status" id="status" type="text" size="30" maxlength="200"value="<?=$arrProjectStatus['status']?>"  class="select" /></td></tr>
              <tr><td valign="top"><?=$LBSummary?>:</td><td><textarea name="description" id="description" cols="50" rows="15"><?=$arrProjectStatus['description']?></textarea></td></tr>
			  ...
			  <tr><td><?=$LBLanguage?>:</td><td>   
								   ...
								   while ($arrLangRow = $christDB->f_GetRecord($resultAll)) { ?>
								    <option value="<?=$arrLangRow['langCode']?>" <?if($arrLangRow['langCode']==$arrProjectStatus['langCode']){echo 'selected';}?>><?=$arrLangRow['langCaption']?></option>
								   <?php } ?>
								   </select>                      
                                    </td>
              </tr>
